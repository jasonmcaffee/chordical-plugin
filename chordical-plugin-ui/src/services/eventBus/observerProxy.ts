...
export function createObserverProxy<T extends object>(root: T){
  ...
  const proxiedEventedProperty: ProxyOfT<RootContainer> = createProxyOfT(eventedProperty, rootContainer);
    ...
    get(_, key: keyof T): FunctionThatReturnsPropertyOfTAsAnEventBusIntersectedWithProxyOfT<T, keyof T>{
      ...
      function getProxyOfPropertyBeingAccessed(): ProxyOfT<T[keyof T]>{ //
        ...
        if(!propertyOfTAsAnEventBus.nestedPropertyProxies[key]){
          ...
          const nestedEventedPropertyForKey = createPropertyOfTAsAnEventBus<T[keyof T]>(nestedObject, key);
